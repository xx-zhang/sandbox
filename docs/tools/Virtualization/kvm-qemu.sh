...
function qemu_func() {
    ...
    if [ ! -f qemu-$qemu_version.tar.xz ]; then
        wget -c -N "https://download.qemu.org/qemu-$qemu_version.tar.xz"
        wget -c -N "https://download.qemu.org/qemu-$qemu_version.tar.xz.sig"
    ...
    # some checks may be depricated, but keeping them for compatibility with old versions
        ...
        # ToDo reintroduce it?
            ...
            #    # --enable-sparse
                ...
                #    QTARGETS=""
                #fi
                ./configure --target-list=i386-softmmu,x86_64-softmmu,i386-linux-user,x86_64-linux-user --prefix=/usr --libexecdir=/usr/lib/qemu --localstatedir=/var --bindir=/usr/bin/ --enable-gnutls --enable-docs --enable-gtk --enable-vnc --enable-vnc-sasl --enable-vnc-png --enable-vnc-jpeg --enable-curl --enable-kvm  --enable-linux-aio --enable-cap-ng --enable-vhost-net --enable-vhost-crypto --enable-spice --enable-usb-redir --enable-lzo --enable-snappy --enable-bzip2 --enable-coroutine-pool --enable-libxml2 --enable-tcmalloc --enable-replication --enable-tools --enable-capstone
            elif [ "$OS" = "Darwin" ]; then
